...
func (c *ConsensusMyBft) ProcessBlockMessage(msg *model.BlockMessage) {
	...
	key := string(id)
	if err != nil {
		fmt.Printf("hash calculate error")
	}
	var m map[string](chan *model.BlockMessage)
	pip, ok := m[key]
	if ok {
		//说明对应区块已经起协程
		pip <- msg
		...
		//起新的worker线程
		m[key] = make(chan *model.BlockMessage)

		go func(pip chan *model.BlockMessage) {
			cnt := 0
			for {
				<- pip
				c.myBlockProcess(msg)
				//case start
				for {
					msgByte := <-service.BlockConfirmChan
					var msg messages.BlockConfirmMessage
					err := json.Unmarshal(msgByte, &msg)
					if err != nil {
						logger.Warningf("[Node.Run] json.Unmarshal error=%v", err)
					...
					if cnt >= 2 * service2.CertificateAuthorityX509.GetF() + 1 {
						break
					}
				}
			}
			fmt.Println("2f+1 end")
		}(m[key])
	}
...
func (c *ConsensusMyBft) myBlockProcess(msg *model.BlockMessage) {
	id, err := msg.Block.Hash()
	if err != nil {
